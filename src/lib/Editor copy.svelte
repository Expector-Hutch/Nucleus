...
<script lang="ts">
    ...
    export async function getLang(ext) {
        ...
        if (language) {
            ...
            if (!mode) {
                warn(
                    `Syntax highlighting not supported for ${language.name}.`,
                    { file: "Editor.svelte", line: 46 }
                );
            } else {
                setLangMode(mode);
            }
        ...
        warn(`Language not found or extension "${ext}" not supported.`, {
            file: "Editor.svelte",
            line: 53,
        });
        return "Unknown";
    ...
    export function setTheme() {
        editorView.dispatch({
            effects: theme.reconfigure([
                get(editorTheme),
                syntaxHighlighting(get(editorHighlightStyle)),
            ]),
        });
    }
    export function setFontFamily(family: string) {
        editorView.dispatch({
            effects: fontFamily.reconfigure(
                EditorView.theme({
                    "*": { fontFamily: family },
                })
            ),
                ...
                extensions: [
                    ...
                    indentationMarkers({
                        ...
                        colors: {
                            ...
                            activeDark: getThemeProperty(
                                "editor-activeIndentLineColor"
                            ),
                            light: getThemeProperty("editor-gutterForeground"),
                            activeLight: getThemeProperty(
                                "editor-activeIndentLineColor"
                            ),
                        },
                    }),
                    ...
                    EditorView.updateListener.of(async (update) => {
                        ...
                        if (
                            update.state.selection.ranges.some((r) => !r.empty)
                        ) {
                            updateLineInfo();
                        }
                    }),
    ...
    async function updateContent() {
        ...
        if ((await appSettings.get("editor.autosave")) === "true") {
            ...
            _ = setTimeout(async () => {
                if (
                    !$file_info.path ||
                    $file_info.path === $file_info.filename ||
                    $file_info.path === ""
                ) {
    ...
    export function updateLineInfo() {
        let lineNumber = editorView.state.doc.lineAt(
            editorView.state.selection.main.head
        ).number;
        let columnNumber =
            editorView.state.selection.ranges[0].head -
            editorView.state.doc.lineAt(editorView.state.selection.main.head)
                .from;
        line_info.set({
            line: lineNumber.toString(),
            column: (columnNumber + 1).toString(),
        });
    }
